/**
 * =============================================================================
 *                    CREATERECRUITERJOBS.JS - Job Creation Script
 * =============================================================================
 *
 * 📖 WHAT IS THIS SCRIPT?
 * ------------------------
 * This script creates 3 jobs (Amazon, Google, Facebook) for the recruiter
 * (admin@gmail.com). It also creates or finds the associated companies and
 * optionally creates a test application for the Amazon job.
 *
 * 🔗 HOW TO RUN:
 * --------------
 *     cd "/Users/ankursura/Desktop/Job Portal Project /backend"
 *     node scripts/createRecruiterJobs.js
 *
 * 📌 WHAT IT DOES:
 * ---------------
 * 1. Connects to MongoDB
 * 2. Finds the recruiter (admin@gmail.com)
...
// Used to find the recruiter and regular users
...
// Line 10: Define async function to create recruiter jobs
// async function = Function that can use await keyword for asynchronous operations
// createRecruiterJobs = Function name
async function createRecruiterJobs() {
  ...
  try {
    ...
    // Find the recruiter (admin@gmail.com)
    // Line 17: Find the recruiter user in database
    ...
    // Line 18: Check if recruiter was found
    ...
    if (!recruiter) {
      ...
      console.error('❌ Recruiter not found. Please make sure admin@gmail.com exists.');
      // Line 20: Exit the script with error code
      // process.exit(1) = Terminate Node.js process with exit code 1 (error)
      process.exit(1);
    }
    // Line 21: Log success message with recruiter details
    // console.log = Output message to terminal
    // Template literal (backticks) = Allows string interpolation with ${}
    // `✅ Found recruiter: ${recruiter.name} (${recruiter.email})` = Shows recruiter name and email
    ...
    for (const companyName of companyNames) {
      ...
      if (!company) {
        ...
        company = await Company.create({
          ...
          name: companyName,
          // Line 32: User ID of the recruiter who owns this company
          // userId = Reference to User document
          // recruiter._id = MongoDB ObjectId of the recruiter user
    ...
    // Line 43: Find a regular user (not the recruiter) to create test application
    // await = Wait for database query to complete
    // User.findOne({ ... }) = Mongoose method to find one document
    //   { email: { $ne: 'admin@gmail.com' }, role: 'user' } = Query criteria:
    //     email: { $ne: 'admin@gmail.com' } = Email not equal to 'admin@gmail.com' ($ne = not equal)
    //     role: 'user' = Role must be 'user' (not 'hirer' or 'admin')
    // regularUser = Variable storing found user document (or null if not found)
    const regularUser = await User.findOne({ email: { $ne: 'admin@gmail.com' }, role: 'user' });
    ...
    const amazonJob = await Job.findOneAndUpdate(
      ...
      { 
        ...
        // Line 53: Hirer ID must match the recruiter's ID
        // hirerId = recruiter._id = Match jobs owned by this specific recruiter
        // recruiter._id = MongoDB ObjectId of the recruiter user
...
- Great work culture and team environment`,
        ...
        // hirerId = recruiter._id = Reference to User document of the recruiter
        // recruiter._id = MongoDB ObjectId of the recruiter user
        hirerId: recruiter._id,
...
// createRecruiterJobs() = Invoke the async function
// This starts the script execution when file is run with: node scripts/createRecruiterJobs.js
createRecruiterJobs();
